  ...
  foreach ( $item2loop as $tcversion_id => $val)
  {
    ...
    { 
      ...
      if(isset($exec_data['createIssue']) && !is_null($issueTracker) && method_exists($issueTracker,'addIssue'))
      {
        ...
        // assembla specific parameters
        if(property_exists($exec_signature,'assembla_assigned_to'))
        {
          $execContext->assembla_assigned_to = $exec_signature->assembla_assigned_to;
        }
        if(property_exists($exec_signature,'assembla_estimate'))
        {
          $execContext->assembla_estimate = $exec_signature->assembla_estimate;
        }
        if(property_exists($exec_signature,'assembla_milestone'))
        {
          $execContext->assembla_milestone = $exec_signature->assembla_milestone;
        }
        if(property_exists($exec_signature,'assembla_reported_by'))
        {
          $execContext->assembla_reported_by = $exec_signature->assembla_reported_by;
        }
        if(property_exists($exec_signature,'attachment'))
        {
          $execContext->attachment = $exec_signature->attachment;
        }
...
function addIssue($dbHandler,$argsObj,$itsObj)
{
  ...
  // assembla specific parameters
  $opt->assembla_assigned_to = $argsObj->assembla_assigned_to;
  $opt->assembla_estimate = $argsObj->assembla_estimate;
  $opt->assembla_milestone = $argsObj->assembla_milestone;
  $opt->assembla_reported_by = $argsObj->assembla_reported_by;
  $opt->attachment = $argsObj->attachment;
  ...
  return array($ret['status_ok'], $ret['msg']);
}
